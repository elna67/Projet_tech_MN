...
void SHOW_IMAGE(const CvMat *pmat, const char str[], int wait)
{
 ...
 cvShowImage(str, mat);
  //cvWaitKey(wait);
  if (cvWaitKey(30) >= 0) EXIT_FAILURE;
  //cvDestroyWindow(str);
  //clear
  cvReleaseMat(&mat);
  //cout << "out\n";
 ...
 */
void mcvLoadImage(CvMat matInit, CvMat **clrImage, CvMat** channelImage)
{
    // load the image
  //IplImage* im;
 ...
 /* cout<<filename<<endl;
  ...
  CvMat temp = matInit;
  //cvGetMat(im, &temp); //PbRun
  *clrImage = cvCloneMat(&temp);
  // convert to single channel
  CvMat *schannel_mat;
  CvMat* tchannelImage = cvCreateMat(temp.height, temp.width, INT_MAT_TYPE);
  //CvMat* tchannelImage = cvCreateMat(im->height, im->width, INT_MAT_TYPE);
  cvSplit(*clrImage, tchannelImage, NULL, NULL, NULL);
  // convert to float
  *channelImage = cvCreateMat(temp.height, temp.width, FLOAT_MAT_TYPE);
