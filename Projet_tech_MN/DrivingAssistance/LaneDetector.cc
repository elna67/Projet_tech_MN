...
#pragma message "still check subtracting mean from image"
  //subtract mean
...
//     }//#endif
  ...
  //debugging
  CvMat *dbIpmImageThresholded = NULL;
  if(DEBUG_LINES) {//#ifdef DEBUG_GET_STOP_LINES
...
#pragma message "Check: Smoothing IPM image"
...
#pragma message "Check this clearing of IPM image for 2 lanes"
  ...
  CvMat *fipm = cvCloneMat(ipm);
    ...
    //zero out negative values
//     SHOW_MAT(fipm, "fipm");
#pragma message "clean negative parts in filtered image"
  mcvThresholdLower(ipm, ipm, 0);
    ...
    for(int i=0; i<(int)splines.size(); i++)
    {
          ...
          || splineStatus & HorizontalSpline))
      {
        ...
        /*CvMat* p = mcvExtendPoints(rawipm, points,
                ...
                //mcvPostprocessLinesineConf->extendIPMLinePixelsTangent,
        ...
        */
		...
		//save
#pragma message "Check this later: extension in IPM. Check threshold value"
// 		splines[i] = spline;
...
//	LaneDetectorParserInfo stopLineParserInfo;
	...
	//  assert(LaneDetectorParser_configfile(fileName, &stopLineParserInfo, 0, 1, 1)==0);
	//init the strucure
	stopLineConf->ipmWidth = IPM_WIDTH;
	stopLineConf->ipmHeight = IPM_HEIGHT;
	stopLineConf->ipmLeft = IPM_LEFT;
	stopLineConf->ipmRight = IPM_RIGHT;
	stopLineConf->ipmBottom = IPM_BOTTOM;
	stopLineConf->ipmTop = IPM_TOP;
	stopLineConf->ipmInterpolation = IPM_INTERPOLATION;

	stopLineConf->lineWidth = LINE_WIDTH;
	stopLineConf->lineHeight = LINE_HEIGHT;
	stopLineConf->kernelWidth = KERNEL_WIDTH;
	stopLineConf->kernelHeight = KERNEL_HEIGHT;
	stopLineConf->lowerQuantile =
		LOWER_QUANTILE;
	stopLineConf->localMaxima =
		LOCAL_MAXIMA;
	stopLineConf->groupingType = GROUPING_TYPE;
	stopLineConf->binarize = BINARIZE;
	stopLineConf->detectionThreshold =
		DETECTION_THRESHOLD;
	stopLineConf->smoothScores =
		SMOOTH_SCORES;
	stopLineConf->rMin = R_MIN;
	stopLineConf->rMax = R_MAX;
	stopLineConf->rStep = R_STEP;
	stopLineConf->thetaMin = THETA_MIN * CV_PI / 180;
	stopLineConf->thetaMax = THETA_MAX * CV_PI / 180;
	stopLineConf->thetaStep = THETA_STEP * CV_PI / 180;
	stopLineConf->ipmVpPortion = IPM_VP_PORTION;
	stopLineConf->getEndPoints = GET_END_POINTS;
	stopLineConf->group = GROUP;
	stopLineConf->groupThreshold = GROUP_THRESHOLD;
	stopLineConf->ransac = RANSAC;

	stopLineConf->ransacLineNumSamples = RANSAC_LINE_NUM_SAMPLES;
	stopLineConf->ransacLineNumIterations = RANSAC_LINE_NUM_ITERATIONS;
	stopLineConf->ransacLineNumGoodFit = RANSAC_LINE_NUM_GOOD_FIT;
	stopLineConf->ransacLineThreshold = RANSAC_LINE_THRESHOLD;
	stopLineConf->ransacLineScoreThreshold = RANSAC_LINE_SCORE_THRESHOLD;
	stopLineConf->ransacLineBinarize = RANSAC_LINE_BINARIZE;
	stopLineConf->ransacLineWindow = RANSAC_LINE_WINDOW;

	stopLineConf->ransacSplineNumSamples = RANSAC_SPLINE_NUM_SAMPLES;
	stopLineConf->ransacSplineNumIterations = RANSAC_SPLINE_NUM_ITERATIONS;
	stopLineConf->ransacSplineNumGoodFit = RANSAC_SPLINE_NUM_GOOD_FIT;
	stopLineConf->ransacSplineThreshold = RANSAC_SPLINE_THRESHOLD;
	stopLineConf->ransacSplineScoreThreshold = RANSAC_SPLINE_SCORE_THRESHOLD;
	stopLineConf->ransacSplineBinarize = RANSAC_SPLINE_BINARIZE;
	stopLineConf->ransacSplineWindow = RANSAC_SPLINE_WINDOW;

	stopLineConf->ransacSplineDegree = RANSAC_SPLINE_DEGREE;

	stopLineConf->ransacSpline = RANSAC_SPLINE;
	stopLineConf->ransacLine = RANSAC_LINE;
	stopLineConf->ransacSplineStep = RANSAC_SPLINE_STEP;

	stopLineConf->overlapThreshold = OVERLAP_THRESHOLD;

	stopLineConf->localizeAngleThreshold = LOCALIZE_ANGLE_THRESHOLD;
	stopLineConf->localizeNumLinePixels = LOCALIZE_NUM_LINE_PIXELS;

	stopLineConf->extendAngleThreshold = EXTEND_ANGLE_THRESHOLD;
	stopLineConf->extendMeanDirAngleThreshold = EXTEND_MEAN_DIR_ANGLE_THRESHOLD;
	stopLineConf->extendLinePixelsTangent = EXTEND_LINE_PIXELS_TANGENT;
	stopLineConf->extendLinePixelsNormal = EXTEND_LINE_PIXELS_NORMAL;
	stopLineConf->extendContThreshold = EXTEND_CONT_THRESHOLD;
	stopLineConf->extendDeviationThreshold = EXTEND_DEVIATION_THRESHOLD;
	stopLineConf->extendRectTop = EXTEND_RECT_TOP;
	stopLineConf->extendRectBottom = EXTEND_RECT_BOTTOM;

	stopLineConf->extendIPMAngleThreshold = EXTEND_IPM_ANGLE_THRESHOLD;
	stopLineConf->extendIPMMeanDirAngleThreshold = EXTEND_IPM_MEAN_DIR_ANGLE_THRESHOLD;
	stopLineConf->extendIPMLinePixelsTangent = EXTEND_IPM_LINE_PIXELS_TANGENT;
	stopLineConf->extendIPMLinePixelsNormal = EXTEND_IPM_LINE_PIXELS_NORMAL;
	stopLineConf->extendIPMContThreshold = EXTEND_IPM_CONT_THRESHOLD;
	stopLineConf->extendIPMDeviationThreshold = EXTEND_IPM_DEVIATION_THRESHOLD;
	stopLineConf->extendIPMRectTop = EXTEND_IPM_RECT_TOP;
	stopLineConf->extendIPMRectBottom = EXTEND_IPM_RECT_BOTTOM;

	stopLineConf->splineScoreJitter = SPLINE_SCORE_JITTER;
	stopLineConf->splineScoreLengthRatio = SPLINE_SCORE_LENGTH_RATIO;
	stopLineConf->splineScoreAngleRatio = SPLINE_SCORE_ANGLE_RATIO;
	stopLineConf->splineScoreStep = SPLINE_SCORE_STEP;

	stopLineConf->splineTrackingNumAbsentFrames = SPLINE_TRACKING_NUM_ABSENT_FRAMES;
	stopLineConf->splineTrackingNumSeenFrames = SPLINE_TRACKING_NUM_SEEN_FRAMES;

	stopLineConf->mergeSplineThetaThreshold = MERGE_SPLINE_THETA_THRESHOLD;
	stopLineConf->mergeSplineRThreshold = MERGE_SPLINE_R_THRESHOLD;
	stopLineConf->mergeSplineMeanThetaThreshold = MERGE_SPLINE_MEAN_THETA_THRESHOLD;
	stopLineConf->mergeSplineMeanRThreshold = MERGE_SPLINE_MEAN_R_THRESHOLD;
	stopLineConf->mergeSplineCentroidThreshold = MERGE_SPLINE_CENTROID_THRESHOLD;

	stopLineConf->lineTrackingNumAbsentFrames = LINE_TRACKING_NUM_ABSENT_FRAMES;
	stopLineConf->lineTrackingNumSeenFrames = LINE_TRACKING_NUM_SEEN_FRAMES;

	stopLineConf->mergeLineThetaThreshold = MERGE_LINE_THETA_THRESHOLD;
	stopLineConf->mergeLineRThreshold = MERGE_LINE_R_THRESHOLD;

	stopLineConf->numStrips = NUM_STRIPS;


	stopLineConf->checkSplines = CHECK_SPLINES;
	stopLineConf->checkSplinesCurvenessThreshold = CHECK_SPLINES_CURVENESS_THRESHOLD;
	stopLineConf->checkSplinesLengthThreshold = CHECK_SPLINES_LENGTH_THRESHOLD;
	stopLineConf->checkSplinesThetaDiffThreshold = CHECK_SPLINES_THETA_DIFF_THRESHOLD;
	stopLineConf->checkSplinesThetaThreshold = CHECK_SPLINES_THETA_THRESHOLD;

	stopLineConf->checkIPMSplines = CHECK_IPM_SPLINES;
	stopLineConf->checkIPMSplinesCurvenessThreshold = CHECK_IPM_SPLINES_CURVENESS_THRESHOLD;
	stopLineConf->checkIPMSplinesLengthThreshold = CHECK_IPM_SPLINES_LENGTH_THRESHOLD;
	stopLineConf->checkIPMSplinesThetaDiffThreshold = CHECK_IPM_SPLINES_THETA_DIFF_THRESHOLD;
	stopLineConf->checkIPMSplinesThetaThreshold = STOP_CHECK_IPM_SPLINES_THETA_THRESHOLD;

	stopLineConf->finalSplineScoreThreshold = FINAL_SPLINE_SCORES_THRESHOLD;

	stopLineConf->useGroundPlane = USE_GROUND_PLANE;

	stopLineConf->checkColor = CHECK_COLOR;
	stopLineConf->checkColorNumBins = CHECK_COLOR_NUM_BINS;
	stopLineConf->checkColorWindow = CHECK_COLOR_WINDOW;
	stopLineConf->checkColorNumYellowMin = CHECK_COLOR_NUM_YELLOW_MIN;
	stopLineConf->checkColorRGMin = CHECK_COLOR_RG_MIN;
	stopLineConf->checkColorRGMax = CHECK_COLOR_RG_MAX;
	stopLineConf->checkColorGBMin = CHECK_COLOR_GB_MIN;
	stopLineConf->checkColorRBMin = CHECK_COLOR_RB_MIN;
	stopLineConf->checkColorRBFThreshold = CHECK_COLOR_RBF_THRESHOLD;
	stopLineConf->checkColorRBF = CHECK_COLOR_RBF;

	stopLineConf->ipmWindowClear = IPM_WINDOW_CLEAR;
	stopLineConf->ipmWindowLeft = IPM_WINDOW_LEFT;
	stopLineConf->ipmWindowRight = IPM_WINDOW_RIGHT;

	stopLineConf->checkLaneWidth = CHECK_LANE_WIDTH;
	stopLineConf->checkLaneWidthMean = CHECK_LANE_WIDTH_MEAN;
	stopLineConf->checkLaneWidthStd = CHECK_LANE_WIDTH_STD;
}

void mcvInitLaneDetectorConf2(char * const fileName,
	LaneDetectorConf *stopLineConf)
{
	//parsed camera data
	//	LaneDetectorParserInfo stopLineParserInfo;
	...
	//  assert(LaneDetectorParser_configfile(fileName, &stopLineParserInfo, 0, 1, 1)==0);
	//init the strucure
	stopLineConf->ipmWidth = STOP_IPM_WIDTH;
	stopLineConf->ipmHeight = STOP_IPM_HEIGHT;
	stopLineConf->ipmLeft = STOP_IPM_LEFT;
	stopLineConf->ipmRight = STOP_IPM_RIGHT;
	stopLineConf->ipmBottom = STOP_IPM_BOTTOM;
	stopLineConf->ipmTop = STOP_IPM_TOP;
	stopLineConf->ipmInterpolation = STOP_IPM_INTERPOLATION;

	stopLineConf->lineWidth = STOP_LINE_WIDTH;
	stopLineConf->lineHeight = STOP_LINE_HEIGHT;
	stopLineConf->kernelWidth = STOP_KERNEL_WIDTH;
	stopLineConf->kernelHeight = STOP_KERNEL_HEIGHT;
	stopLineConf->lowerQuantile =
		LOWER_QUANTILE;
	stopLineConf->localMaxima =
		LOCAL_MAXIMA;
	stopLineConf->groupingType = STOP_GROUPING_TYPE;
	stopLineConf->binarize = STOP_BINARIZE;
	stopLineConf->detectionThreshold =
		DETECTION_THRESHOLD;
	stopLineConf->smoothScores =
		SMOOTH_SCORES;
	stopLineConf->rMin = STOP_R_MIN;
	stopLineConf->rMax = STOP_R_MAX;
	stopLineConf->rStep = STOP_R_STEP;
	stopLineConf->thetaMin = STOP_THETA_MIN * CV_PI / 180;
	stopLineConf->thetaMax = STOP_THETA_MAX * CV_PI / 180;
	stopLineConf->thetaStep = STOP_THETA_STEP * CV_PI / 180;
	stopLineConf->ipmVpPortion = STOP_IPM_VP_PORTION;
	stopLineConf->getEndPoints = STOP_GET_END_POINTS;
	stopLineConf->group = STOP_GROUP;
	stopLineConf->groupThreshold = STOP_GROUP_THRESHOLD;
	stopLineConf->ransac = STOP_RANSAC;

	stopLineConf->ransacLineNumSamples = STOP_RANSAC_LINE_NUM_SAMPLES;
	stopLineConf->ransacLineNumIterations = STOP_RANSAC_LINE_NUM_ITERATIONS;
	stopLineConf->ransacLineNumGoodFit = STOP_RANSAC_LINE_NUM_GOOD_FIT;
	stopLineConf->ransacLineThreshold = STOP_RANSAC_LINE_THRESHOLD;
	stopLineConf->ransacLineScoreThreshold = STOP_RANSAC_LINE_SCORE_THRESHOLD;
	stopLineConf->ransacLineBinarize = STOP_RANSAC_LINE_BINARIZE;
	stopLineConf->ransacLineWindow = STOP_RANSAC_LINE_WINDOW;

	stopLineConf->ransacSplineNumSamples = STOP_RANSAC_SPLINE_NUM_SAMPLES;
	stopLineConf->ransacSplineNumIterations = STOP_RANSAC_SPLINE_NUM_ITERATIONS;
	stopLineConf->ransacSplineNumGoodFit = STOP_RANSAC_SPLINE_NUM_GOOD_FIT;
	stopLineConf->ransacSplineThreshold = STOP_RANSAC_SPLINE_THRESHOLD;
	stopLineConf->ransacSplineScoreThreshold = STOP_RANSAC_SPLINE_SCORE_THRESHOLD;
	stopLineConf->ransacSplineBinarize = STOP_RANSAC_SPLINE_BINARIZE;
	stopLineConf->ransacSplineWindow = STOP_RANSAC_SPLINE_WINDOW;

	stopLineConf->ransacSplineDegree = STOP_RANSAC_SPLINE_DEGREE;

	stopLineConf->ransacSpline = STOP_RANSAC_SPLINE;
	stopLineConf->ransacLine = STOP_RANSAC_LINE;
	stopLineConf->ransacSplineStep = STOP_RANSAC_SPLINE_STEP;

	stopLineConf->overlapThreshold = STOP_OVERLAP_THRESHOLD;

	stopLineConf->localizeAngleThreshold = STOP_LOCALIZE_ANGLE_THRESHOLD;
	stopLineConf->localizeNumLinePixels = STOP_LOCALIZE_NUM_LINE_PIXELS;

	stopLineConf->extendAngleThreshold = STOP_EXTEND_ANGLE_THRESHOLD;
	stopLineConf->extendMeanDirAngleThreshold = STOP_EXTEND_MEAN_DIR_ANGLE_THRESHOLD;
	stopLineConf->extendLinePixelsTangent = STOP_EXTEND_LINE_PIXELS_TANGENT;
	stopLineConf->extendLinePixelsNormal = STOP_EXTEND_LINE_PIXELS_NORMAL;
	stopLineConf->extendContThreshold = STOP_EXTEND_CONT_THRESHOLD;
	stopLineConf->extendDeviationThreshold = STOP_EXTEND_DEVIATION_THRESHOLD;
	stopLineConf->extendRectTop = STOP_EXTEND_RECT_TOP;
	stopLineConf->extendRectBottom = STOP_EXTEND_RECT_BOTTOM;

	stopLineConf->extendIPMAngleThreshold = STOP_EXTEND_IPM_ANGLE_THRESHOLD;
	stopLineConf->extendIPMMeanDirAngleThreshold = STOP_EXTEND_IPM_MEAN_DIR_ANGLE_THRESHOLD;
	stopLineConf->extendIPMLinePixelsTangent = STOP_EXTEND_IPM_LINE_PIXELS_TANGENT;
	stopLineConf->extendIPMLinePixelsNormal = STOP_EXTEND_IPM_LINE_PIXELS_NORMAL;
	stopLineConf->extendIPMContThreshold = STOP_EXTEND_IPM_CONT_THRESHOLD;
	stopLineConf->extendIPMDeviationThreshold = STOP_EXTEND_IPM_DEVIATION_THRESHOLD;
	stopLineConf->extendIPMRectTop = STOP_EXTEND_IPM_RECT_TOP;
	stopLineConf->extendIPMRectBottom = STOP_EXTEND_IPM_RECT_BOTTOM;

	stopLineConf->splineScoreJitter = STOP_SPLINE_SCORE_JITTER;
	stopLineConf->splineScoreLengthRatio = STOP_SPLINE_SCORE_LENGTH_RATIO;
	stopLineConf->splineScoreAngleRatio = STOP_SPLINE_SCORE_ANGLE_RATIO;
	stopLineConf->splineScoreStep = STOP_SPLINE_SCORE_STEP;

	stopLineConf->splineTrackingNumAbsentFrames = STOP_SPLINE_TRACKING_NUM_ABSENT_FRAMES;
	stopLineConf->splineTrackingNumSeenFrames = STOP_SPLINE_TRACKING_NUM_SEEN_FRAMES;

	stopLineConf->mergeSplineThetaThreshold = STOP_MERGE_SPLINE_THETA_THRESHOLD;
	stopLineConf->mergeSplineRThreshold = STOP_MERGE_SPLINE_R_THRESHOLD;
	stopLineConf->mergeSplineMeanThetaThreshold = STOP_MERGE_SPLINE_MEAN_THETA_THRESHOLD;
	stopLineConf->mergeSplineMeanRThreshold = STOP_MERGE_SPLINE_MEAN_R_THRESHOLD;
	stopLineConf->mergeSplineCentroidThreshold = STOP_MERGE_SPLINE_CENTROID_THRESHOLD;

	stopLineConf->lineTrackingNumAbsentFrames = STOP_LINE_TRACKING_NUM_ABSENT_FRAMES;
	stopLineConf->lineTrackingNumSeenFrames = STOP_LINE_TRACKING_NUM_SEEN_FRAMES;

	stopLineConf->mergeLineThetaThreshold = STOP_MERGE_LINE_THETA_THRESHOLD;
	stopLineConf->mergeLineRThreshold = STOP_MERGE_LINE_R_THRESHOLD;

	stopLineConf->numStrips = STOP_NUM_STRIPS;


	stopLineConf->checkSplines = STOP_CHECK_SPLINES;
	stopLineConf->checkSplinesCurvenessThreshold = STOP_CHECK_SPLINES_CURVENESS_THRESHOLD;
	stopLineConf->checkSplinesLengthThreshold = STOP_CHECK_SPLINES_LENGTH_THRESHOLD;
	stopLineConf->checkSplinesThetaDiffThreshold = STOP_CHECK_SPLINES_THETA_DIFF_THRESHOLD;
	stopLineConf->checkSplinesThetaThreshold = STOP_CHECK_SPLINES_THETA_THRESHOLD;

	stopLineConf->checkIPMSplines = STOP_CHECK_IPM_SPLINES;
	stopLineConf->checkIPMSplinesCurvenessThreshold = STOP_CHECK_IPM_SPLINES_CURVENESS_THRESHOLD;
	stopLineConf->checkIPMSplinesLengthThreshold = STOP_CHECK_IPM_SPLINES_LENGTH_THRESHOLD;
	stopLineConf->checkIPMSplinesThetaDiffThreshold = STOP_CHECK_IPM_SPLINES_THETA_DIFF_THRESHOLD;
	stopLineConf->checkIPMSplinesThetaThreshold = STOP_CHECK_IPM_SPLINES_THETA_THRESHOLD;

	stopLineConf->finalSplineScoreThreshold = STOP_FINAL_SPLINE_SCORES_THRESHOLD;

	stopLineConf->useGroundPlane = STOP_USE_GROUND_PLANE;

	stopLineConf->checkColor = STOP_CHECK_COLOR;
	stopLineConf->checkColorNumBins = STOP_CHECK_COLOR_NUM_BINS;
	stopLineConf->checkColorWindow = STOP_CHECK_COLOR_WINDOW;
	stopLineConf->checkColorNumYellowMin = STOP_CHECK_COLOR_NUM_YELLOW_MIN;
	stopLineConf->checkColorRGMin = STOP_CHECK_COLOR_RG_MIN;
	stopLineConf->checkColorRGMax = STOP_CHECK_COLOR_RG_MAX;
	stopLineConf->checkColorGBMin = STOP_CHECK_COLOR_GB_MIN;
	stopLineConf->checkColorRBMin = STOP_CHECK_COLOR_RB_MIN;
	stopLineConf->checkColorRBFThreshold = STOP_CHECK_COLOR_RBF_THRESHOLD;
	stopLineConf->checkColorRBF = STOP_CHECK_COLOR_RBF;

	stopLineConf->ipmWindowClear = STOP_IPM_WINDOW_CLEAR;
	stopLineConf->ipmWindowLeft = STOP_IPM_WINDOW_LEFT;
	stopLineConf->ipmWindowRight = STOP_IPM_WINDOW_RIGHT;

	stopLineConf->checkLaneWidth = STOP_CHECK_LANE_WIDTH;
	stopLineConf->checkLaneWidthMean = STOP_CHECK_LANE_WIDTH_MEAN;
	stopLineConf->checkLaneWidthStd = STOP_CHECK_LANE_WIDTH_STD;
}
                      ...
                      float *lineAbc)
{
  ...
  cout<< "display cols of cpoints "<< cpointst->cols<<endl;
  cout<< "display cols of W "<< W->cols<<endl;
  cout<< "display cols of V"<< V->cols<<endl;

  cout<< "display rows of cpoints "<< cpointst->rows<<endl;
...
#pragma message "Using weighted sampling for Ransac Line"
                       ...
                       LineType lineType)
{
  ...
  for (int i=0; i<(int)boxes.size(); i++) //lines
  {
    ...
    switch (lineType)
    {
      ...
      {
        //get extent
        //int ystart = (int)max(min(line.startPoint.y, line.endPoint.y)-window, 0);
        //int yend = (int)min(max(line.startPoint.y, line.endPoint.y)+window, height-1);
        int ystart = (int)max(box.y - window, 0);
        int yend = (int)min(box.y + box.height + window, height-1);
        //get the mask
      ...
      case LINE_VERTICAL:
      {
        ...
        //int xstart = (int)max(min(line.startPoint.x, line.endPoint.x)-window, 0);
        //int xend = (int)min(max(line.startPoint.x, line.endPoint.x)+window, width-1);
        int xstart = (int)max(box.x - window, 0);
        int xend = (int)min(box.x + box.width + window, width-1);
        //get the mask
...
#pragma message "check this screening in ransacLines"
    if (lineRTheta[0]>=0)
    {
      bool put =true;
      switch(lineType)
  ...
  lines.clear();
  lineScores.clear();
#pragma message "not grouping at end of getRansacLines"
...
void  mcvSetMat(CvMat *inMat, CvRect mask, double val)
{
  ...
  //(int)max(min(line.startPoint.x, line.endPoint.x)-xwindow, 0);
  int xstart = mask.x, xend = mask.x + mask.width-1;
  //xend = (int)min(max(line.startPoint.x, line.endPoint.x), width-1);
...
/*
  SHOW_MAT(points,"after");
  cout << "AFTER SORTING !" <<endl;
  cout<< "height points "<< points->height<<endl;
  cout << "points->rows "<<points->rows<<endl;
  cout <<"points->width "<<points->width<<endl;
  cout<< "points->cols " << points->cols<<endl;
  cout<<"data.db"<< points->data.db<<endl;
  cout<<"data.fl"<< points->data.fl<<endl;
  cout<<"data.i"<< points->data.i<<endl;
  cout<<"data.ptr"<< points->data.ptr<<endl;
  cout<<"data.s"<< points->data.s<<endl;*/
 ...
 // cout << "p0(x):" << CV_MAT_ELEM(*points, float, 0, 0) << endl;
 // cout << "p0(y):" << CV_MAT_ELEM(*points, float, 0, 1) << endl;


  float diff = 0.f;

 // cout << " pour créer us :points->height : " << points->height << endl;
  float *us = new float[points->height];
  us[0] = 0;

  for (int i=1; i<points->height; ++i)
  {
   // cout << "i = "<<i<<"_____________________________"<<endl;
     // cout << "dx = CV_MAT_ELEM(*points, float, i, 0)(" <<CV_MAT_ELEM(*points, float, i-1, 0)<<") - CV_MAT_ELEM(*points, float, i-1, 0) (" << CV_MAT_ELEM(*points, float, i-1, 0) <<");"<<endl;
...
Spline mcvFitBezierSplinefromPoints(CvMat *points, int degree)
{
                                ...
                                bool extendSpline)
{
  ...
  CvMat *pixelst0 = NULL, *pixelst1 = NULL;
  if (extendSpline)
  {
                         ...
                         vector<float> &splineScores, LineState* state)
{
  ...
  //try grouping the lines into regions
  //float groupThreshold = 15;
#pragma message "no line grouping in getRansacSplines"
...
#pragma message "Turned off adding boxes from previous frame"
                        ...
                        vector<Spline> *prevSplines)
{
  ...
  //subtract half
#pragma message "check adding half to points"
  CvMat p;
...
void mcvDrawSpline(CvMat *image, Spline spline, CvScalar color, int width)
{
  ...
  for (int i = 0; i < pixels->height - 1; i++)
	  ...
	  // 		color);
  {
	  double x1 = cvGetReal2D(pixels, i, 0);
	  double y1 = cvGetReal2D(pixels, i, 1);
	  double x2 = cvGetReal2D(pixels, i+1, 0);
	  double y2 = cvGetReal2D(pixels, i+1, 1);
  ...
  if (meanTheta && meanR)
  {
    ...
    *meanR = 0;
    ...
    for (int i=0; i<points->height-1; i++)
    {
	    ...
	    mcvLineXY2RTheta(line, r, t);
	    //add pi if neg
#pragma message "add pi to theta calculations for spline feature"
	    //if (t<0) t += CV_PI;
	    //add
  ...
  switch(type)
  {
    case LINE_VERTICAL:
      for(unsigned int i=0; i<splines.size(); ++i)
      {
        ...
        start = (int)min(splines[i].points[0].x,
                          splines[i].points[splines[i].degree].x);
        end = (int)max(splines[i].points[0].x,
                        splines[i].points[splines[i].degree].x);
    ...
    case LINE_HORIZONTAL:
      for(unsigned int i=0; i<splines.size(); ++i)
      {
        ...
        start = (int)min(splines[i].points[0].y,
                          splines[i].points[splines[i].degree].y);
        end = (int)max(splines[i].points[0].y,
                        splines[i].points[splines[i].degree].y);
                              ...
                              CvSize size, vector<CvRect> &boxes)
{
  ...
  switch(type)
  {
    case LINE_VERTICAL:
      ...
      {
        //get min and max x and add the bounding box covering the whole height
        start = (int)min(lines[i].startPoint.x, lines[i].endPoint.x);
        end = (int)max(lines[i].startPoint.x, lines[i].endPoint.x);
        boxes.push_back(cvRect(start, 0, end-start+1, size.height-1));
      }
      break;
    ...
    case LINE_HORIZONTAL:
      ...
      {
        //get min and max y and add the bounding box covering the whole width
        start = (int)min(lines[i].startPoint.y, lines[i].endPoint.y);
        end = (int)max(lines[i].startPoint.y, lines[i].endPoint.y);
        boxes.push_back(cvRect(0, start, size.width-1, end-start+1));
      }
      break;
    }
