...
//  assert(cameraInfoParser_configfile(fileName, &camInfo, 0, 1, 1)==0);
  ...
  cameraInfo->focalLength.x = FOCAL_LENGTH_X;
  cameraInfo->focalLength.y = FOCAL_LENGTH_Y;
  cameraInfo->opticalCenter.x = OPTICAL_CENTER_X;
  cameraInfo->opticalCenter.y = OPTICAL_CENTER_Y;
  cameraInfo->cameraHeight = CAMERA_HEIGHT;
  cameraInfo->pitch = PITCH * CV_PI / 180;
  cameraInfo->yaw = YAW * CV_PI / 180;
  cameraInfo->imageWidth = IMAGE_WIDTH;
  cameraInfo->imageHeight = IMAGE_HEIGHT;
}
